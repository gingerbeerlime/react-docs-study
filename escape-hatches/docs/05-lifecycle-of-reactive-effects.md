...
`effects`는 리액트 컴포넌트의 생명주기처럼 항상 마운트 될 때 동기화를 시작하고 마운트 해제될 때 동기화가 중지되는 것은 아니다. `effects`는 `props`나 `state` 와 같은 **반응형 값의 변화에 반응해서 동작**한다.
...
- 의존성 값을 **“선택”**할 수 없기 때문에 `effect`가 **너무 자주 다시 동기화되거나 무한루프가 발생**한다면 아래 항목들을 체크해보기
